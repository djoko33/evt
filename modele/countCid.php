...
function top10() {
	...
	include('connexionPtirex.php');
	$result=array();
	$lstCodes=$ptirex->prepare("SELECT code, COUNT(*) as nb FROM codescid WHERE (dateevt BETWEEN ? AND ?) GROUP BY code ORDER BY nb DESC LIMIT 10");
...
function countCodeTrim($code) {
	...
	$lstNbCVT=$ptirex->prepare("SELECT (EXTRACT (QUARTER from dateevt)) as trim, (EXTRACT (YEAR from dateevt)) as annee,".
			" COUNT(reference) as nb from codescid WHERE (code=?) AND (dateevt BETWEEN ? AND ?) GROUP BY trim, annee ORDER BY annee, trim");
